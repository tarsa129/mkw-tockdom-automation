...
def read_slot_text(section_text):
    advice_type_regex = "^(It is (recommended|mandatory|not recommended) to put this (track|arena)|This (track|arena) (will work|will not work|will only work)) on the \[\[Slot#((battle)?[1-8]\.[1-4])\|[a-zA-Z<\/> ]{0,40}slot]](, \[\[Slot#((battle)?[1-8]\.[1-4])\|[a-zA-Z<\/> ]{0,40}slot]])?(, \[\[Slot#((battle)?[1-8]\.[1-4])\|[a-zA-Z<\/> ]{0,40}slot]])?(,? or \[\[Slot#((battle)?[1-8]\.[1-4])\|[a-zA-Z<\/> ]{0,40}slot]])?( in ([^,]{0,10})(( to ([^,]{0,10}))|((, ([^,]{0,10}))?(, ([^,]{0,10}))?(,? and (.{0,10}))?)))?( (because of|for) (the .{0,120}))?\.( If used in a distribution that supports custom music slots, it is recommended to use the \[\[Slot#((battle)?[1-8]\.[1-4])\|[a-zA-Z<\/> ]{0,40}slot]](, \[\[Slot#((battle)?[1-8]\.[1-4])\|[a-zA-Z<\/> ]{0,40}slot]])?(, \[\[Slot#((battle)?[1-8]\.[1-4])\|[a-zA-Z<\/> ]{0,40}slot]])?(,? or \[\[Slot#((battle)?[1-8]\.[1-4])\|[a-zA-Z<\/> ]{0,40}slot]])?\.)?$"
